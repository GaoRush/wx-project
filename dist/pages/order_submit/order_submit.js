...
Page({
  ...
  onLoad: function onLoad(options) {
    ...
    if (options.id_list) {
      ...
      var gb_urlData = JSON.parse(options.gb_urlData);
  ...
  onShow: function onShow() {
    console.log('onShow');
    var that = this;
    var address_selected = wx.getStorageSync('address_selected');
    ...
    if (address_selected) {
      ...
      var isGroupBuying = wx.getStorageSync('isGroupBuying');

      if (isGroupBuying == true) {
        var gb_urlData = wx.getStorageSync('gb_urlData');
        // 团购结算
        that.gb_buy_now(gb_urlData);
        console.log('团购结算', gb_urlData);
      } else {
        var buy_now = wx.getStorageSync('buy_now');
        ...
        if (buy_now == 1) {
          ...
          var id_list = wx.getStorageSync('id_list');
          // 购物车结算
          that.cart_submit(id_list);
  ...
  // 1.支付方式
  payment_type_radioChange: function payment_type_radioChange(e) {
    console.log('radio发生change事件，携带value值为：', e.detail.value);
    var that = this;

    console.log('payment_type', that.data.payment_type);
    var payment_type = e.detail.value;
    if (payment_type == 2) {
      wx.showModal({
        title: '支付提示',
        content: '是否线下支付',
        success: function success(res) {
          if (res.confirm) {
            ...
            that.setData((_that$setData = {
              payment_type: payment_type
            }, _defineProperty(_that$setData, 'payment_type_items[0].checked', true), _defineProperty(_that$setData, 'payment_type_items[1].checked', false), _that$setData));
            // return false
          }
        }
      });
  ...
  //2.选择优惠券
  bindCheckCoupon: function bindCheckCoupon(e) {
    ...
    // 优惠券编号
    var coupon_no = that.data.coupon_no;
    // 选中的优惠券张数
    var checked_num_coupon = that.data.checked_num_coupon;
    // 选中的优惠券总金额
    var checked_money_coupon = Number(that.data.checked_money_coupon);
  ...
  //2.1选择优惠券关闭按钮
  btn_coupon_pop: function btn_coupon_pop() {
    var that = this;
  ...
  //3.是否使用钱包
  is_wallet: function is_wallet(e) {
    var that = this;
    console.log('is_wallet', e.detail.value);
    console.log('that', that.data);
    var is_wallet = e.detail.value ? 1 : 0;
  ...
  bindInput_money: function bindInput_money(e) {
    ...
    if (name == 'use_money') {
      ...
      var use_money = Number(e.detail.value).toFixed(2);
      // 正则检测是否正确金额，否取最大值金额
  ...
  //4.计算总额
  sum_totalPrice: function sum_totalPrice() {
    var that = this;
    // 商品总额
    var order_money = that.data.order_money;
    // 优惠券总额
    var checked_money_coupon = that.data.checked_money_coupon;
    // 钱包使用总额
    var use_money = Number(that.data.use_money);

    var totalPrice = order_money - checked_money_coupon - use_money;
  ...
  //支付流程：1.购物车结算
  cart_submit: function cart_submit(id_list) {
  ...
  order_pre_submit: function order_pre_submit() {
    var that = this;
    login.requestP_pro({
      ...
      if (that.data.address.mobile) {
        var mobile = that.data.address.mobile;
        mobile = common.hide_mobile(mobile);
        that.setData(_defineProperty({}, 'address.mobile', mobile));
      }
      ...
      if (that.data.coupon_list) {
        ...
        if (coupon_list.length > 0) {
          for (var i = 0; i < coupon_list.length; i++) {
            var coupon_item = coupon_list[i];
            var start_time = common.GetDateFormat(coupon_item.start_time);
            var end_time = common.GetDateFormat(coupon_item.end_time);
  ...
  order_submit: function order_submit() {
    var that = this;
    var address_id = that.data.address_id;
    var payment_type = that.data.payment_type;
    var coupon_no = that.data.coupon_no.replace(',', '');
    var is_wallet = that.data.is_wallet;
    var use_money = Number(that.data.use_money);
  ...
  gb_buy_now: function gb_buy_now() {
    ...
    var that = this;
    var urlData = wx.getStorageSync('gb_urlData');
    console.log('urlData', urlData);
    login.requestP_pro({
  ...
  group_buying_order_pre_submit: function group_buying_order_pre_submit() {
  ...
  group_buying_order_submit: function group_buying_order_submit() {
    var that = this;
    var address_id = that.data.address_id;
    var payment_type = that.data.payment_type;
    var is_wallet = that.data.is_wallet;
