      ...
      success: function (res) {
        ...
        wx.uploadFile({
          ...
          success: function (res) {
            ...
            const data = res.data;
            const res_json = JSON.parse(res.data);

            // status==1，上传成功
            if (res_json.status == 1) {
              console.log('上传头像', res_json)
              let img = config.url + res_json.path
              console.log('img', img)
              that.setData({
                [`userInfo.avatar_url`]: img
              })
              console.log('that.data', that.data)
